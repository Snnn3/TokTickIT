...
Version: 2.6 | Date: 2026-09-19 | Companion to `specification.md` (FR/BR/AC refs).
...
* Change-password gate: if `user.mustChangePassword=true`, every endpoint except `GET /api/auth/me`, `POST /api/auth/change-password` and `POST /api/auth/logout` returns `403 PASSWORD_CHANGE_REQUIRED`. Two paths are deliberate exceptions to that gate, and to authentication itself: `GET /api/health` stays public (anonymous `200` and gated-user `200`) because a liveness probe must answer whether the process is up, not who is asking; and the Lab 1 compatibility endpoint `GET /api/categories` stays public — anonymous `200` and gated-user (`mustChangePassword=true`) `200` — because it was never closed to begin with (BR-29, BR-28), while `GET /api/reference/categories` and `GET /api/reference/systems` stay authenticated and gated.
...
* **No cookie is required.** Logout returns `204` + a cleared `Set-Cookie` on every path where the session is known to be dead — no cookie, an invalid cookie, a successful revocation, or a stale-version replay that matched zero rows. This is deliberate: a client whose cookie has already expired or been invalidated must still be able to complete a sign-out and reach a clean state, and returning `401` there would strand the user on a screen whose only escape is the action that just failed.
* The `500` path is the deliberate exception (AC-06, FR-30, BR-20): when the `tokenVersion` revocation fails or the session cannot be verified (DB/config fault), the server answers `500 UNEXPECTED` with **no** `Set-Cookie` clear and the client retains its local session for a retry. The presented cookie stays valid for up to eight hours, so clearing here would report a live session as signed out while a replay still yields `200` on `GET /api/auth/me`.
* Response `204` + cleared `Set-Cookie`, no body — except `500 { "error": { "code": "UNEXPECTED", "message": "Failed to complete the sign-out" } }` with no clear on DB/config failure.
