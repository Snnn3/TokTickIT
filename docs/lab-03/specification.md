...
* **BR-28** Lab 2 tests fall into three groups under this migration. **Retired** tests assert behaviour the sprint deletes outright — the development-requester listing endpoint, the Requester Selection screen and the selector-only evidence captures — and remain retired rather than skipped. **Adapted** tests keep their subject and assertions while changing identity to the session cookie: the Lab 2 server/client suites are adapted, and `e2e/lab-02/requester-ticket-flow.spec.ts` now preserves the requester ticket, ownership, attachment and responsive regression under authenticated sessions. **Unchanged** tests are everything else. AC-17 asserts all three: retired selector-only tests stay out of the runnable suite, every adapted test passes, and every unchanged test passes untouched. `tests.md` records which file sits in each group and why.
* **BR-29** Exception to the authenticated-by-default matrix: the Lab 1 compatibility endpoint `GET /api/categories` remains unauthenticated. It exposes only active category names, which are not user data, and BR-28 preserves the Lab 1 test that asserts an anonymous 200 — protecting this path would retire a test the disposition keeps. The authenticated reference endpoints (`GET /api/reference/categories`, `GET /api/reference/systems`) are what the Lab 3 client uses.
...
The matrix covers authenticated callers. **An unauthenticated caller receives `401 AUTH_REQUIRED` on every row above except login**, including the reference endpoints — those return active categories and systems, which are internal service-desk configuration rather than public data, so they are authenticated like everything else. Only `POST /api/auth/login` is reachable without a session, with the sole exception of the Lab 1 compatibility endpoint `GET /api/categories` (BR-29).
...
* [x] UI conforms to `ui-spec.md` (tokens, badges, states, responsive, accessibility), covered by the 135 passing client tests, 13 passing browser tests (3 requester regression + 10 release tests) and the release-state screenshots.
...
* [x] README setup, run and test instructions current, including seeded credentials, with the session-adapted `e2e/lab-02` regression and the retired selector-only capture specs documented alongside the Lab 3 browser evidence.
