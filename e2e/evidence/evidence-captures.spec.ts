...
async function fillValidForm(page: Page, summary: string) {
  ...
  await page.locator("#description-input").fill("Evidence capture ticket for submission screenshots.");
}
...
test("submission evidence captures", async ({ page, request }) => {
  ...
  const labelOf = (p: { name: string; email: string }) => `${p.name} (${p.email})`;
  ...
  await page.screenshot({ path: path.join(EVIDENCE, "01-validation-failure.png") });
  ...
  await expect(page.getByTestId("file-errors")).toContainText(/unsupported format/i);
  await page.screenshot({ path: path.join(EVIDENCE, "04-invalid-attachment-exe.png") });
  ...
  await expect(page.getByTestId("file-errors")).toContainText(/exceeds maximum allowed size/i);
  await page.screenshot({ path: path.join(EVIDENCE, "05-invalid-attachment-oversize.png") });
  await page.getByRole("button", { name: "Dismiss warning" }).click();
  ...
  await page.screenshot({ path: path.join(EVIDENCE, "02-submitting-busy.png") });
  await expect(page.getByTestId("success-panel")).toBeVisible({ timeout: 15000 });
  await page.unroute("**/api/tickets", slowPost);
  const ticketNumber = await page.getByTestId("success-ticket-number").innerText();
  ...
  await page.screenshot({ path: path.join(EVIDENCE, "06-backend-down-banner-preserved.png") });
  ...
  await page.screenshot({ path: path.join(EVIDENCE, "07a-isolation-A-sees-ticket.png") });
  ...
  await page.screenshot({ path: path.join(EVIDENCE, "07b-isolation-B-sees-nothing.png") });
  ...
  await page.screenshot({ path: path.join(EVIDENCE, "09-no-results-state.png") });
  ...
  await expect(page.getByText(/Access denied. You do not have permission/)).toBeVisible();
  await page.screenshot({ path: path.join(EVIDENCE, "10-forbidden-403-cross-access.png") });
  ...
  await page.getByTestId("removal-reason-input").fill("Evidence: superseded version.");
  ...
  await expect(page.getByTestId(/^removed-reason-/).first()).toContainText("superseded version");
  await page.screenshot({ path: path.join(EVIDENCE, "13-removed-row.png") });
  const removedBadge = page.getByTestId(/^removed-badge-/).first();
  const removedAttachmentId = (await removedBadge.getAttribute("data-testid"))?.replace(
    "removed-badge-",
    "",
  );
  ...
  const forbiddenRes = await request.get(`${API}/api/tickets/${evidenceTicketId}`, {
